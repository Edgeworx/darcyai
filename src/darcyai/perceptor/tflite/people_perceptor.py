...
class PoseEngine():
    ...
    def __init__(self,
                 model_path,
                 num_threads: int = 4,
                 mirror=False, arch=os.uname().machine):
        ...
        tflite_runtime = load_delegate = import_module(
            "tflite_runtime.interpreter")
        Interpreter = tflite_runtime.Interpreter

        script_dir = os.path.dirname(os.path.realpath(__file__))

        self._interpreter = Interpreter(model_path, num_threads=num_threads)
        ...
        self._input_height = self._interpreter.get_input_details()[
            0]['shape'][1]
        self._input_width = self._interpreter.get_input_details()[
            0]['shape'][2]

    def detect_poses_in_image(self,
        ...
        if is_dynamic_shape_model:
            resized_image, _ = self._keep_aspect_ratio_resizer(
        ...
        return self._postprocess(model_output, image_height, image_width,
                                 detection_threshold), inf_time

    def _keep_aspect_ratio_resizer(self, image, target_size):
        """Resizes the image.
    ...
    def _postprocess(self, keypoints_with_scores: np.ndarray, image_height: int,
        ...
        for idx in range(num_instances):
            ...
            poses.append(Pose(pose_keypoints, scores))

        return poses


class PeoplePerceptor(TFLitePerceptorBase):
    def __init__(self, **kwargs):
        super().__init__(model_path="")

        self.config_schema = [
            Config("minimum_face_threshold", "float",
                   0.4, "Minimum Face Threshold"),
            Config("minimum_body_threshold", "float",
                   0.2, "Minimum Body Threshold"),
            Config("minimum_face_height", "int", 20, "Minimum Face Height"),
            Config("minimum_body_height", "int", 120, "Minimum Body Height"),
            Config("show_pose_landmark_dots", "bool",
                   False, "Show pose landmark dots"),
            Config("show_body_rectangle", "bool",
                   False, "Show body rectangle"),
            Config("show_face_rectangle", "bool",
                   False, "Show face rectangle"),
            Config("face_rectangle_color", "str",
                   "255,0,0", "Face rectangle color"),
            Config("face_rectangle_thickness", "int",
                   1, "Face rectangle thickness"),
            Config("body_rectangle_color", "str",
                   "0,255,0", "Body rectangle color"),
            Config("pose_landmark_dot_confidence_threshold", "float",
                   0.5, "Pose landmark dot confidence threshold"),
            Config("pose_landmark_dot_size", "int",
                   1, "Pose landmark dot size"),
            Config("pose_landmark_dot_color", "str",
                   "255,255,255", "Pose landmark dot color"),
            Config("show_face_position_arrow", "bool",
                   False, "Show face position arrow"),
            Config("face_position_arrow_color", "str",
                   "255,255,255", "Face position arrow color"),
            Config("face_position_arrow_stroke", "int",
                   1, "Face position arrow stroke"),
            Config("face_position_arrow_offset_x", "int",
                   0, "Face position arrow offset x"),
            Config("face_position_arrow_offset_y", "int", -
                   30, "Face position arrow offset y"),
            Config("face_position_arrow_length", "int",
                   20, "Face position arrow length"),
            Config("face_position_left_right_threshold", "float",
                   0.3, "Face position left/right threshold"),
            Config("face_position_straight_threshold", "float",
                   0.7, "Face position straight threshold"),
            Config("show_forehead_center_dot", "bool",
                   False, "Show forehead center dot"),
            Config("forehead_center_dot_color", "str",
                   "255,255,255", "Forehead center dot color"),
            Config("forehead_center_dot_size", "int",
                   1, "Forehead center dot size"),
            Config("body_rectangle_thickness", "int",
                   1, "Body rectangle thickness"),
            Config("face_rectangle_y_factor", "float",
                   1.0, "Face rectangle Y factor"),
            Config("show_centroid_dots", "bool", False, "Show centroid dots"),
            Config("centroid_dots_color", "str",
                   "255,255,255", "Centroid dots color"),
            Config("centroid_dots_size", "int", 1, "Centroid dots size"),
            Config("object_tracking_allowed_missed_frames", "int",
                   50, "Object tracking allowed missed frames"),
            Config("object_tracking_color_sample_pixels", "int",
                   4, "Object tracking color sample pixels"),
            Config("object_tracking_info_history_count", "int",
                   3, "Object tracking info history count"),
            Config("object_tracking_removal_count", "int",
                   50, "Object tracking removal count"),
            Config("object_tracking_centroid_weight", "float",
                   0.25, "Object tracking centroid weight"),
            Config("object_tracking_color_weight", "float",
                   0.25, "Object tracking color weight"),
            Config("object_tracking_vector_weight", "float",
                   0.25, "Object tracking vector weight"),
            Config("object_tracking_size_weight", "float",
                   0.25, "Object tracking size weight"),
            Config("object_tracking_creation_m", "int",
                   10, "Object tracking creation M"),
            Config("object_tracking_creation_n", "int",
                   7, "Object tracking creation N"),
            Config("person_tracking_creation_m", "int",
                   20, "Person tracking creation M"),
            Config("person_tracking_creation_n", "int",
                   16, "Person tracking creation N"),
            Config("show_person_id", "bool", False, "Show person ID"),
            Config("person_data_line_color", "str",
                   "255,255,255", "Person data line color"),
            Config("person_data_line_thickness", "int",
                   1, "Person data line thickness"),
            Config("person_data_identity_text_color", "str",
                   "255,255,255", "Person data identity text color"),
            Config("person_data_identity_text_stroke", "int",
                   1, "Person data identity text stroke"),
            Config("person_data_identity_text_font_size", "float",
                   1.0, "Person data identity text font size"),
            Config("person_data_text_offset_x", "int",
                   30, "Person data text offset X"),
            Config("person_data_text_offset_y", "int", -
                   40, "Person data text offset Y"),
            Config("identity_text_prefix", "str",
                   "Person ID: ", "Identity text prefix"),
            Config("face_height_resize_factor", "float",
                   0.1, "Face height resize factor"),
            Config("rolling_video_storage_frame_count", "int",
                   100, "Rolling video storage frame count")
        ]
    ...
    def run(self, input_data, config):
        ...
        poses, inference_time = self.__primary_pose_engine.detect_poses_in_image(
            frame)
        ...
        if self.__poi != None and ('person_id' in self.__poi) and self.__poi['person_id'] != 0:
            if self.__prior_poi_id != self.__poi['person_id']:
                self.__set_person_as_poi(
                    self.__poi['person_id'], self.__frame_number, self.__poi, config)
    ...
    def load(self, accelerator_idx: [int, None]) -> None:
        ...
        model_file = os.path.join(script_dir, "models/movenet.tflite")
    ...
    def __process_body_attributes(self, bodies, frame, config):
        for body in bodies:
            body["face_position"] = self.__determine_face_position(
                body, config)
            body["has_forehead"] = self.__determine_if_forehead_visible(
                body, config)
            body["forehead_center"] = self.__determine_forehead_center(
                body, config)
            ...
            curFaceHeight = int(body["face_rectangle"]
                                [1][1] - body["face_rectangle"][0][1])
    ...
    def __get_qualified_body_detections(self, poses, config):
        ...
        for pose in poses:
            ...
            faceScore = (pose.keypoints[0].score + pose.keypoints[1].score +
                         pose.keypoints[2].score + pose.keypoints[3].score + pose.keypoints[4].score) / 5
            bodyScore = (pose.keypoints[5].score + pose.keypoints[6].score + pose.keypoints[7].score + pose.keypoints[8].score + pose.keypoints[9].score + pose.keypoints[10].score +
                         pose.keypoints[11].score + pose.keypoints[12].score + pose.keypoints[13].score + pose.keypoints[14].score + pose.keypoints[15].score + pose.keypoints[16].score) / 12
            ...
            if meetsConfidence:
                ...
                faceHeight = ((pose.keypoints[5].point[1] + pose.keypoints[6].point[1]) / 2) - (
                    (pose.keypoints[1].point[1] + pose.keypoints[2].point[1]) / 2)
                bodyHeight = (
                    (pose.keypoints[15].point[1] + pose.keypoints[16].point[1]) / 2) - pose.keypoints[0].point[1]
    ...
    def __draw_face_position_arrow_on_frame(self, frame, body, config):
        ...
        if body["face_position"] == "Straight":
            startY = noseY - (config.face_position_arrow_length /
                              2) + config.face_position_arrow_offset_y
            endY = noseY + (config.face_position_arrow_length /
                            2) + config.face_position_arrow_offset_y
            ...
            startX = noseX + (config.face_position_arrow_length /
                              2) + config.face_position_arrow_offset_x
            endX = noseX - (config.face_position_arrow_length /
                            2) + config.face_position_arrow_offset_x
            start_point = (int(startX), int(startY))
            end_point = (int(endX), int(endY))
        ...
        cv2.arrowedLine(frame, start_point, end_point, self.__parse_color_string(
            config.face_position_arrow_color), config.face_position_arrow_stroke)
        return frame
    ...
    def __draw_forehead_center_dot_on_frame(self, frame, body, config):
        if body["has_forehead"] and body["forehead_center"] != (0, 0):
            ...
            cv2.circle(frame, (foreheadX, foreheadY), config.forehead_center_dot_size, self.__parse_color_string(
                config.forehead_center_dot_color), config.forehead_center_dot_size)
    ...
    def __draw_body_rectangle_on_frame(self, frame, body, config):
        if body["has_body"] and body["body_rectangle"] != ((0, 0), (0, 0)):
            ...
            cv2.rectangle(frame, upperLeft, lowerRight, self.__parse_color_string(
                config.body_rectangle_color), config.body_rectangle_thickness)
    ...
    def __draw_landmark_points_on_body(self, frame, body, config):
        for label, keypoint in body["pose"].keypoints.items():
            if keypoint.score < config.pose_landmark_dot_confidence_threshold:
                continue
            cv2.circle(frame, (int(keypoint.point[0]), int(keypoint.point[1])), config.pose_landmark_dot_size, self.__parse_color_string(
                config.pose_landmark_dot_color), config.pose_landmark_dot_size)
    ...
    def __find_face_rectangle(self, body, config):
        ...
        faceRectangle = ((int(lowestX), int(lowestY)),
                         (int(highestX), int(highestY)))
    ...
    def __draw_centroid_circles_on_frame(self, frame, body, config):
        ...
        cv2.putText(frame, text, (centroid[0] - 10, centroid[1] - 10), cv2.FONT_HERSHEY_SIMPLEX,
                    0.5, self.__parse_color_string(config.centroid_dots_color), 2, cv2.LINE_AA)
        cv2.circle(frame, (centroid[0], centroid[1]), config.centroid_dots_size, self.__parse_color_string(
            config.centroid_dots_color), config.centroid_dots_size)
        return frame
    ...
    def __draw_face_rectangle_on_frame(self, frame, body, config):
        if body["has_face"] and body["face_rectangle"] != ((0, 0), (0, 0)):
            ...
            cv2.rectangle(frame, upperLeft, lowerRight, self.__parse_color_string(
                config.face_rectangle_color), config.face_rectangle_thickness)
        return frame
    ...
    def __find_body_rectangle(self, body, config):
        ...
        if body["has_body"]:
            ...
            bodyRectangle = ((int(lowestX - 2), int(lowestY - 2)),
                             (int(highestX + 2), int(highestY + 2)))
    ...
    def __determine_face_position(self, body, config):
        ...
        rightEyeRightEarScore = (
            pose.keypoints[2].score + pose.keypoints[4].score) / 2
        eyeAndNoseScore = (
            pose.keypoints[0].score + pose.keypoints[2].score + pose.keypoints[1].score) / 3
    ...
    def __determine_forehead_center(self, body, config):
        ...
        if body["has_forehead"]:
            ...
            endConstant = distanceToForehead / \
                math.sqrt(1 + (inverseEyeSlope * inverseEyeSlope))
            foreheadX = 0
            foreheadY = 0
    ...
    def __record_body_info_in_tracker_for_id(self, frameNumber, bodyID, bodyInfo, config):
        ...
        self.__record_body_seen_value_for_current_frame(
            frameNumber, bodyID, True, config)
    ...
    def __mark_unmatched_body_ids_as_missing(self, currentFrameNumber, config):
        ...
        for bodyID in list(self.__body_history.keys()):
            if currentFrameNumber in self.__body_history[bodyID]:
                ...
                self.__record_body_seen_value_for_current_frame(
                    currentFrameNumber, bodyID, False, config)
    ...
    def __generate_tracking_info_for_body(self, frame, body, config):
        ...
        centroidColorSampleStartX = max(
            centroidX - int(config.object_tracking_color_sample_pixels / 2), 0)
        centroidColorSampleEndX = min(
            centroidX + int(config.object_tracking_color_sample_pixels / 2), self.__frame_width)
        centroidColorSampleStartY = max(
            centroidY - int(config.object_tracking_color_sample_pixels / 2), 0)
        centroidColorSampleEndY = min(
            centroidY + int(config.object_tracking_color_sample_pixels / 2), self.__frame_height)

        centroidColorSampleStartX = min(
            centroidColorSampleStartX, (self.__frame_width - 1))
        centroidColorSampleEndX = max(centroidColorSampleEndX, 1)
        centroidColorSampleStartY = min(
            centroidColorSampleStartY, (self.__frame_height - 1))
        centroidColorSampleEndY = max(centroidColorSampleEndY, 1)

        colorSampleChunk = frame[centroidColorSampleStartY: centroidColorSampleEndY,
                                 centroidColorSampleStartX: centroidColorSampleEndX]
        ...
        for body in bodies:
            ...
            for existingBodyID in list(self.__body_history.keys()):
                ...
                for historyFrameNumber in list(self.__body_history[existingBodyID].keys()):
                    ...
                    if historyNum > 1:
                        vectorX += (self.__body_history[existingBodyID]
                                    [historyFrameNumber]["centroid"][0] - priorX)
                        vectorY += (self.__body_history[existingBodyID]
                                    [historyFrameNumber]["centroid"][1] - priorY)
                    ...
                    cumCentroidDistance += self.__get_euclidean_distance_between_two_points(
                        self.__body_history[existingBodyID][historyFrameNumber]["centroid"], trackingInfo["centroid"], 2)
                    cumColorDistance += self.__get_euclidean_distance_between_two_points(
                        self.__body_history[existingBodyID][historyFrameNumber]["color"], trackingInfo["color"], 3)
                    cumSizeDistance += self.__get_euclidean_distance_between_two_points(
                        self.__body_history[existingBodyID][historyFrameNumber]["size"], trackingInfo["size"], 2)
                ...
                cumCentroidWithVectorDistance = self.__get_euclidean_distance_between_two_points(
                    (projectedX, projectedY), trackingInfo["centroid"], 2)
                ...
                totalCurrentScore = (config.object_tracking_centroid_weight * cumCentroidDistance) + (config.object_tracking_color_weight * cumColorDistance) + (
                    config.object_tracking_vector_weight * cumCentroidWithVectorDistance) + (config.object_tracking_size_weight * cumSizeDistance)
        ...
        for existingBodyID in list(self.__body_history.keys()):
            ...
            if objBestBodyIterator > -1:
                self.__record_body_info_in_tracker_for_id(
                    currentFrameNumber, existingBodyID, bodies[objBestBodyIterator]["tracking_info"], config)
                bodies[objBestBodyIterator]["body_id"] = existingBodyID

                # Check if we have a person data record yet
                hasPerson = self.__check_if_person_data_record_exists_for_body_id(
                    existingBodyID)
                ...
                if hasPerson:
                    ...
                    personReady = self.__determine_if_body_seen_enough_to_create_person(
                        currentFrameNumber, existingBodyID, config)
                    self.__create_new_person_data_record_with_body_id(
                        existingBodyID, config)
                    bodies[objBestBodyIterator]["person_id"] = existingBodyID
    ...
    def __draw_person_data_on_frame(self, frame, body, config):
        ...
        identityText = "{}{}".format(
            config.identity_text_prefix, personIdValue)
        ...
        if config.show_person_id:
            (idWidth, idHeight), idBaseLine = cv2.getTextSize(identityText, cv2.FONT_HERSHEY_SIMPLEX,
                                                              config.person_data_identity_text_font_size, config.person_data_identity_text_stroke)
        ...
        if config.person_data_text_offset_y <= 0:
            lineEndY = foreheadCenter[1] + \
                config.person_data_text_offset_y - halfHeight
            textStartY = foreheadCenter[1] + \
                config.person_data_text_offset_y - totalTextHeightWithBaselines
        else:
            lineEndY = foreheadCenter[1] + \
                config.person_data_text_offset_y + halfHeight
        ...
        cv2.line(frame, foreheadCenter, (lineEndX, lineEndY), self.__parse_color_string(
            config.person_data_line_color), config.person_data_line_thickness)

        if config.show_person_id:
            idWriteY = textStartY + idHeight + idBaseLine
            cv2.putText(frame, identityText, (lineEndX, idWriteY), cv2.FONT_HERSHEY_SIMPLEX, config.person_data_identity_text_font_size,
                        self.__parse_color_string(config.person_data_identity_text_color), config.person_data_identity_text_stroke, cv2.LINE_AA)
